...
const createResource = async (req, res) => {
    try {
        const resource = await Resource.create(setDistrictOnCreate(req, req.body));
...
const getAllResources = async (req, res) => {
    try {
        const resources = await Resource.find(scopedFilter(req))
            .sort({
                createdAt: -1
            });
...
const getResourceById = async (req, res) => {
    try {
        ...
        const resource = await Resource.findOne({ _id: req.params.id, ...scopedFilter(req) });
