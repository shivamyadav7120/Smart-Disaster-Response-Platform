const { scopedFilter, getAdminDistrict } = require("../utils/areaScope");
...
const createSOS = async (req, res) => {
    ...
    try {
        ...
        if (!location || !Number.isFinite(Number(location.latitude)) || !Number.isFinite(Number(location.longitude))) {
            return res.status(400).json({ success: false, message: "A valid GPS latitude and longitude are required to send SOS." });
        }
        location.latitude = Number(location.latitude);
        location.longitude = Number(location.longitude);
        if (location.accuracy != null) location.accuracy = Math.max(0, Number(location.accuracy));
        location.confidence = location.accuracy == null ? "Low" : location.accuracy <= 20 ? "High" : location.accuracy <= 50 ? "Medium" : "Low";
...
const getAllSOS = async (req, res) => {
    ...
    try {
        ...
        const sos = await SOS.find(scopedFilter(req))
