...
export default function RescueTeamTracker() {
  ...
  const sendLatest = async () => {
    ...
    try {
      ...
      const normalizedAccuracy = Number.isFinite(gpsAccuracy) ? gpsAccuracy : null
      await updateMyRescueTeamLocation({ latitude, longitude, accuracy: normalizedAccuracy })
      setLastUpdate(new Date())
      setAccuracy(normalizedAccuracy)
      setConfidence(normalizedAccuracy == null ? "Low" : normalizedAccuracy <= 20 ? "High" : normalizedAccuracy <= 50 ? "Medium" : "Low")
  ...
  const startTracking = () => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        setConfidence(nextAccuracy == null ? "Low" : nextAccuracy <= 20 ? "High" : nextAccuracy <= 50 ? "Medium" : "Low")
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {accuracy != null && <p className="text-[11px] text-slate-500 mt-1">GPS accuracy: ~{Math.round(accuracy)} m · Confidence: <b>{confidence}</b></p>}
