...
export default function WeatherCard() {
  ...
  useEffect(() => {
    ...
    const loadWeather = async () => {
      ...
      try {
        const data = await fetchCurrentAreaWeather();
        ...
        if (!cancelled) {
          ...
          setError(
            err?.code === 1
            ? "Allow browser location access to load live weather."
            : "Live weather unavailable. Check location permission and WEATHER_API_KEY."
          );
        }
      } finally {
        if (!cancelled) {
