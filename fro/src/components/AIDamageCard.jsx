...
export default function AIDamageCard() {
  ...
  const handleUpload = async (e) => {
    ...
    if (!file) return
    setLoading(true)
    try { const result = await detectDamage(file); setReport(result) } catch (err) { console.error(err) } finally { setLoading(false) }
  }
    ...
    <div className="card p-5 bg-white border border-[#dbeadb] rounded-2xl">
      ...
      {loading ? (
        ...
        <div className="flex gap-3 items-center">
          ...
          <div>
            <div className="font-bold text-[15px] text-forest-900">{report?.title || 'No AI report yet'}</div>
