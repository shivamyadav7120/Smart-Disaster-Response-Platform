...
import { fetchCurrentAreaWeather } from "../services/weather";
    ...
    const loadWeather = async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          setError(
            err?.code === 1
              ? "Location permission was denied. Allow location access in your browser to load live weather for your current area."
              : err?.code === 2
              ? "Your current location could not be detected. Check GPS/location settings and try again."
              : err?.code === 3
              ? "Location detection timed out. Please try again."
              : "Unable to load live weather. Check that WEATHER_API_KEY is configured correctly in bac/.env."
          );
        }
      } finally {
        if (mounted) {
